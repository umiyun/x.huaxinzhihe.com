...
define('UMIACP_NAME', 'umiacp_groupprepay2');
...
//        $data['receive_mobile'] = $from['receive_mobile'];
        ...
        $data['goods_val'] = $from['goods_val'];
        $data['goods_imgs'] = serialize($from['goods_imgs']);
        ...
        if ($from['activity_id']) {
            ...
            if(!$cutmember){//已有报名人数不可修改
                $data['jieti'] = serialize($from['jieti']);
                $data['yprice'] = $from['yprice'];
                $data['gnum'] = $from['gnum'];
                $data['oprice'] = $from['oprice'];
            }
