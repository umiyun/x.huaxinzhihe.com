...
define('UMIACP_NAME', 'umiacp_lighten');
...
//        $data['effects_id'] = $from['effects_id'];
        ...
        $data['banner'] = $from['banner'];
        $data['dl_img'] = $from['dl_img'];
        $data['dlactive_img'] = $from['dlactive_img'];
        $data['lnum'] = $from['lnum'];
        $data['gnum'] = $from['gnum'];
...
//        $data['receive_mobile'] = $from['receive_mobile'];
        ...
        if ($from['activity_id']) {
            ...
            if(!$cutmember){//已有报名人数不可修改
                $data['needstatus'] = $from['needstatus'];
                $data['needmoney'] = $from['needmoney'];
                $data['signstatus'] = $from['signstatus'];
                $data['signnum'] = $from['signnum'];
            }else{
                if(intval($from['signnum'])>$activity['signnum']){
                    $data['signnum'] = $from['signnum'];
                }
            }
