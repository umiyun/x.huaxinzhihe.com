...
require_once IA_ROOT . '/addons/umi_activitys/core/defines.php';
require_once IA_ROOT . '/addons/umi_activitys/core/loader.php';

class Umi_activitysModuleSite extends WeModuleSite
{
    ...
    public function __construct()
    {
        ...
        if(($global_task['execute_time']<$task_nowtime&&$global_task['state']==1)||empty($global_task)){
            ihttp_request(YOUMI_URL . "task.php?i=".$this->uniacid, NULL, NULL, 1);
        }
        if ($_W['user']) {
            $this->uid = $_W['uid'];
            ...
            if (in_array($this->user_type, [2, 3])) {
                ...
                if (!$shop && in_array($_GPC['do'], ['user', 'buying', 'making'])) {
        ...
        if ($params['from'] == 'return') {
            ...
            if ($params['result'] == 'success') {
                if ($params['result'] == 'success') {
                    ...
                    message('支付失败！', $this->createMobileUrl('buying'), 'error');
                }
            }
        }
    }
