...
if ($op == 'display') {
    ...
    $cut['vote_imgs']=unserialize($cut['photo']);
    ...
    $cut_my = pdo_get(YOUMI_NAME . '_author', ['activity_id' => $activity_id,'mid' => $this->mid]);
    $activity = pdo_get(YOUMI_NAME . '_activity', ['id' => $activity_id]);

//    名次
    $ranking = pdo_fetchall('select * from ' . tablename(YOUMI_NAME . '_author') .' where uniacid = ' . $uniacid . ' and activity_id = ' . $activity_id .' and status=2 and times > '.$cut['times'].' order by times ASC ');
    $ranking2 = pdo_fetchall('select * from ' . tablename(YOUMI_NAME . '_author') .' where uniacid = ' . $uniacid . ' and activity_id = ' . $activity_id .' and status=2 and times < '.$cut['times'].' order by times desc ');
    if($ranking){
    ...
    $update['pv +='] = 1;
    pdo_update(YOUMI_NAME . '_author', $update, ['id' => $cut_id]);
