...
//    }
    ...
    $cuts=pdo_fetchall('select * from ' . tablename(YOUMI_NAME . '_author') .' where uniacid = ' . $uniacid . ' and activity_id = ' . $activity_id .' and status=2 ');
    ...
    $isvoter=pdo_get(YOUMI_NAME . '_author', ['activity_id' => $activity_id,'mid' => $this->mid]);

    $_W['page']['title'] = $activity['title'] ? $activity['title'] : '世界游记投票活动';
    ...
    $records = pdo_fetchall('select * from ' . tablename(YOUMI_NAME . '_author') .' where uniacid = ' . $uniacid . ' and activity_id = ' . $activity_id .' and status =2 order by times DESC,createtime ASC');
...
//    }
    ...
    if($activity['endtime']<time()&&$activity['qrcode']==0){
        pdo_update(YOUMI_NAME . '_author', ['islottery' => 1], ['activity_id' => $activity_id]);
        pdo_update(YOUMI_NAME . '_activity', ['qrcode' => 1], ['id' => $activity_id]);
        pdo_query('update ims_'.YOUMI_NAME . '_author'." set islottery=2 where activity_id=".$activity_id." order by times DESC limit ".$activity['pnum']);
    }
...
if ($op == 'sign_up') {
    ...
    $records = pdo_fetchall('select * from ' . tablename(YOUMI_NAME . '_author') .' where uniacid = ' . $uniacid . ' and activity_id = ' . $activity_id .' and status = "3" ');

    $cut = pdo_get(YOUMI_NAME . '_author', ['mid' => $this->mid, 'activity_id' => $activity_id]);
    if ($cut) {
    ...
    $cut['photo']=serialize($_GPC['vote_imgs']);

    pdo_insert(YOUMI_NAME . '_author', $cut);
    $cut_id = pdo_insertid();
...
if ($op == 'voting') {
    ...
    $cut = pdo_get(YOUMI_NAME . '_author', ['id' => $cut_id]);
    $activity = pdo_get(YOUMI_NAME . '_activity', ['id' => $activity_id]);
...
//    pdo_update(YOUMI_NAME . '_activity',array('voter'=>($activity['voter']+1)),['id' => $activity['id']]);
    pdo_update(YOUMI_NAME . '_author',array('times'=>($cut['times']+1)),['id' => $cut_id]);
