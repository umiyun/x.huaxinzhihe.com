...
if ($op == 'post') {
    ...
    $voters = pdo_fetchall('select * from ' . tablename(YOUMI_NAME . '_author') . ' where activity_id = :id ', $paras);
...
function addVoteVirtualNum($activity, $cut_id, $addNum,$pv,$share,$uniacid)
{
 ...
 $cut_mid=   pdo_getcolumn(YOUMI_NAME . '_author',['id'=>$cut_id],'mid');
    ...
    pdo_update(YOUMI_NAME . '_author', ['times +=' => $addNum,'pv +='=>$pv,'share'=>$share], ['id'=>$cut_id]);
