...
if ($op == 'display') {
    ...
    $list = pdo_fetchall('SELECT * FROM ' . tablename(YOUMI_NAME . '_' . 'activity_prize') . ' where uniacid = :uniacid and activity_id =:activity_id ' . $condition . $orderby . ' LIMIT ' . ($pindex - 1) * $psize . ',' . $psize, $paras);
    ...
    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename(YOUMI_NAME . '_' . 'activity_prize') . ' WHERE uniacid = :uniacid and activity_id =:activity_id ' . $condition, $paras);
    $pager = pagination($total, $pindex, $psize);
    include $this->template('prize', ['activity_id' => $activity_id]);
}
...
if ($op == 'post') {
    ...
    if (checksubmit('submit')) {
        ...
        itoast('温馨提示：' . $message . '奖品成功！', $this->createWebUrl('activity'), 'success');
    }
    $item = pdo_fetch('select * from ' . tablename(YOUMI_NAME . '_' . 'activity_prize') . ' where id = :id limit 1 ', $paras);

    include $this->template('prize', ['activity_id' => $activity_id, 'id' => $id]);
}
