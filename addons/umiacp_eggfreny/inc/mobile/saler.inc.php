...
$order = pdo_fetch('SELECT * FROM ' . tablename(YOUMI_NAME . '_' . 'prize_log') . ' where uniacid = :uniacid and id = ' . $order_id, [':uniacid' => $uniacid]);
...
if ($op == 'check') {
    ...
    $res = pdo_update(YOUMI_NAME . '_' . 'prize_log', ['status' => 2], ['id' => $order_id]);
    if ($res) {
