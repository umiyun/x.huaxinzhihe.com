...
require_once IA_ROOT . '/addons/umiacp_fission/core/defines.php';
require_once IA_ROOT . '/addons/umiacp_fission/core/loader.php';

class umiacp_fissionModuleSite extends WeModuleSite
{
    ...
    public function __construct()
    {
        ...
        $task_nowtime=strtotime(" -30 seconds ");

        $global_task=pdo_get('umi_activitys_task',array('uniacid'=>$this->uniacid));

        if(($global_task['execute_time']<$task_nowtime&&$global_task['state']==1)||empty($global_task)){
            ihttp_request($_W['siteroot'] . 'addons/umi_activitys/' . "task.php?i=".$this->uniacid, NULL, NULL, 1);
        }
        if ($_W['user']) {
            $this->uid = $_W['uid'];
...
//                }
    ...
    protected function pay($params = array(), $mine = array())
    {
        ...
        if ($order['status'] == 4) {
            pdo_update(YOUMI_NAME . '_cut', ['status' => 4], ['id' => $order['cut_id']]);
            message('订单已取消', '', 'info');
        }
        if ($order['status'] == 2) {
            message('订单已支付', '', 'info');
    ...
    public function payResult($params)
    {
        ...
        if ($params['from'] == 'return') {
            ...
            if ($status) {
                ...
                youmi_settlement_log($order, 1, $order['price']-$order['commission'], '用户支付订单：订单ID：' . $order['id'] . '，用户：' . $this->username . '，支付时间：' . date('Y-m-d H:i:s'));
