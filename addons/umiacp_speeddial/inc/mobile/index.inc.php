...
//        }
    ...
    $_W['page']['title'] = $activity['title'] ? $activity['title'] : '活动宝九宫格抽奖';
...
//        $prize = pdo_get(YOUMI_NAME . '_' . 'activity_prize', array('status' => 0,'activity_id' => $prize['activity_id']));
        $prize = pdo_fetch('select * from ' . tablename(YOUMI_NAME . '_activity_prize') . ' where status = 0 and activity_id = ' . $prize['activity_id'].' order by rand() ');
    } else {
