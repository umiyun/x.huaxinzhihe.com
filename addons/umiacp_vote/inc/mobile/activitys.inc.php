...
if ($op == 'display') {
    ...
    $activity = pdo_fetchall('select * from ' . tablename(YOUMI_NAME . '_activity') . ' where uniacid = ' . $uniacid . ' and id in (select activity_id from ' . tablename(YOUMI_NAME . '_cut') . ' where uniacid = ' . $uniacid . ' and mid = ' . $this->mid . ' group by activity_id  order by createtime desc)');
