...
$order = pdo_fetch('SELECT * FROM ' . tablename(YOUMI_NAME . '_' . 'cut') . ' where uniacid = :uniacid and id = ' . $order_id, [':uniacid' => $uniacid]);
...
if ($op == 'check') {
    ...
    $res = pdo_update(YOUMI_NAME . '_' . 'cut', ['islottery' => 3,'lottery_time' => time()], ['id' => $order_id]);
    if ($res) {
